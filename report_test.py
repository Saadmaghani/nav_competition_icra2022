...
"""
format of out.txt:
    old - <world_id> <success> <collision> <timeout> <time_taken> <metric(BARN only)>
    new - <world_id> <success> <collision> <timeout> <time_taken> <metric(BARN only)> <algo> <timestamp>
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dynabarn', default=False, action="store_true")
    args = parser.parse_args()
    
    base_path = 'jackal_helper'

    if not args.dynabarn:
        optimal_times = {}
        cell_counts = {}
        for idx in range(50):
    ...
    with open(args.out_path, "r") as f:
        for l in f.readlines():
            ...
            if args.dynabarn: # 1_success (optionally we could multiply it by 1/actual_time +eps)
                metric = int(logs[1])*1/(float(logs[4])+1e-5)
            else: # 1_success * optimal_time / clip(actual_time, 2 * optimal_time, 4 * opti * actual_time (but this means closer to 0 the better)al_time)
                metric = int(logs[1]) * optimal_times[world_idx] / np.clip(float(logs[4]), optimal_times[world_idx] * 4, optimal_times[world_idx] * 8)  
    ...
    print("Avg Time on Successful runs: %.4f, Avg Metric[0, 0.25]: %.4f, Avg Success[0, 1]: %.4f, Avg Collision[0, 1]: %.4f, Avg Timeout[0, 1]: %.4f" %(
        np.mean(mean_time),
